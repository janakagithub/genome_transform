package genome_transform::genome_transformServer;
...
sub call_method {
    ...
    my $ctx = genome_transform::genome_transformServerContext->new($self->{loggers}->{userlog},
...
{
    # Service genome_transform requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for genome_transform but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = genome_transform::genome_transformServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service genome_transform.");
    }
...
package genome_transform::genome_transformServerContext;

use strict;

=head1 NAME

genome_transform::genome_transformServerContext
...
package genome_transform::genome_transformServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = genome_transform::genome_transformServer->new(
